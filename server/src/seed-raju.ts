...
const conversationsToCreate = [
  ...
  // Group conversations
  { name: 'Family Group', is_group: true, participants: ['raju', 'suresh', 'priya'] },
  { name: 'Office Team', is_group: true, participants: ['raju', 'ananya', 'rahul', 'neha'] },
];
...
const messagesToCreate = [
  ...
  // Messages for Raju and Priya
  {
    ...
    messages: [
      ...
      { text: "Glad you like it. Let me know if you need any changes", sender: 'priya' },
  ...
  // Messages for Raju and Ananya
  {
    ...
    messages: [
      ...
      { text: "It was great! Went hiking with family. How about you?", sender: 'raju' },
  ...
  // Messages for Family Group
  {
    ...
    messages: [
      ...
      { text: "Sounds like a productive day ahead for everyone!", sender: 'priya' },
    ]
  },
...
async function seedRajuData() {
  ...
  try {
    ...
    for (const userData of usersToCreate) {
      ...
      if (!user) {
        user = await User.create({
          ...
          password_hash: '$2b$10$abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789',
          display_name: userData.display_name,
          about: userData.about,
          online: false,
          last_seen: 'recently',
        });
        console.log(`Created user: ${userData.display_name} (${userData.username})`);
      } else {
        console.log(`Found existing user: ${userData.display_name} (${userData.username})`);
      }
    ...
    for (const convData of conversationsToCreate) {
      ...
      console.log(`Created ${convData.is_group ? 'group' : 'individual'} conversation with ID: ${conversation.id}`);
      ...
      for (const username of convData.participants) {
        ...
        if (user) {
          ...
          console.log(`Added ${user.display_name} as ${role} to conversation ${conversation.id}`);
        }
      }
    }
    ...
    for (const convMessages of messagesToCreate) {
      ...
      for (let i = 0; i < convMessages.messages.length; i++) {
        ...
        if (senderUser) {
          ...
          messageTime.setMinutes(baseTime.getMinutes() - convMessages.messages.length + i);
      ...
      console.log(`Added ${convMessages.messages.length} messages to conversation ${conversation.id}`);
    }
