...
async function checkRajaData() {
  ...
  try {
    ...
    const conversations = Array.from(
      new Set(conversationParticipants.map((cp) => cp.conversation.id)),
    ).map(
      (id) =>
        conversationParticipants.find((cp) => cp.conversation.id === id)
          ?.conversation,
    );
    ...
    for (const conversation of conversations) {
      ...
      const participants = conversation.participants
        .map(
          (p: ConversationParticipant & { user: User }) => p.user.display_name,
        )
        .join(', ');
